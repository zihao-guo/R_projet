...
DT::datatable(fr)
```
Voici une carte montrant la proportion de personnes contaminés dans chaque pays. On remarque que les régions les plus touchées sont l'Europe et l'Amérique, alors que l'Asie, l'Afrique et L'océanie sont moins touchées.
...
df1<- data %>% 
  ...
  mutate(rate_confirmed= confirmed/population) %>%  
  select(rate_confirmed,administrative_area_level_1) %>% 
  slice(which.max(rate_confirmed))

confirmed_world_map<-
right_join(world_map, df1, by = c("region" = "administrative_area_level_1"))

ggplot(confirmed_world_map, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill=rate_confirmed), color = "white") +
  ...
  scale_fill_gradient(low = "yellow", high="red") +labs(title="Carte de l'épidémie mondiale",y="Latitude", x="Longitude",subtitle = "Taux d'infection (cas confirmés/population)\n") +
  theme_light() 
```
