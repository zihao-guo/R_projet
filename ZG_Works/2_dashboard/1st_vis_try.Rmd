...
z_t %>% write.csv(file = "../../A_Datas/age_2.csv", row.names=F)
```
...
data3 %>% write.csv(file = "../../A_Datas/age_3.csv", row.names=F)
```


```{r}
data4 <- data3 %>% 
  filter(patient_gender!=1) %>% 
  select(-(patient_gender)) %>% 
  mutate(id = c(1:6))

ggplot(data=data4, aes(x=id, y=mean)) +
  geom_polygon(color = "black", fill= "#E41A1C",alpha=0.1)+
  geom_point(size=2,shape=10,color = 'black', fill= "#E41A1C")+
  coord_polar() +
